...
| Prepared statements | Complete | driver-selected `Statement(D)`; explicit direct/pool owners; session-monotonic PostgreSQL names; unsigned OID metadata; narrow one-shot invalidation recovery | SQLite schema-change tests and PostgreSQL protocol/direct/pool/cache tests |
