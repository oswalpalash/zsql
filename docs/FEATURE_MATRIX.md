...
| Native PostgreSQL driver | Complete | full-open `connect_timeout`; structurally parsed SCRAM attributes, allocator-bounded salt handling, and strict constant-time raw verifier comparison; endpoint-owning `CancelHandle`; owned notifications; clean pooled listeners; direct and short-lease pooled COPY with owned output; lifecycle-aware command recovery | SCRAM extension/ambiguity/error/base64 and exhaustive allocation-failure tests; cancellation/notification/COPY ownership and OOM tests; live post-teardown ownership, clean pool reuse, cancellation/COPY/notification recovery in CI |
