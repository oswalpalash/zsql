...
| Optional offline query checks | Complete within documented bounded scope | `zsql.check`, `zsql.checkedQuery`; exact bind contracts; typed domain wrappers; authoritative numeric width checks | parser/shape/type/narrowing tests; `zig build check-sql` |
