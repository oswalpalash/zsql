...
| Connection pooling | Complete | `Pool(D)`, `Lease(D)`, allocator-owned connection configuration, health-aware consuming release, PostgreSQL destructive protocol teardown outside the accounting mutex, opt-in PostgreSQL `DISCARD ALL` borrower isolation with pipelined restoration and a single asynchronous-event boundary, OOM-safe idle return and owned-result unwind, failed-open capacity wakeup, shutdown draining/wakeup, stats and timeouts | Cross-driver config-clone OOM/deferred-shutdown and deterministic failed-open infinite-waiter tests; SQLite release-OOM/result-unwind/recovery tests; deterministic PostgreSQL stalled-peer mutex-liveness and stale-reset-notification regressions; PostgreSQL live session-state reset, reset-OOM consumption, release-OOM/result-unwind/shutdown tests |
