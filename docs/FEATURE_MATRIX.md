...
| Query builder | Complete | `QueryBuilder`; reusable failure-free reset; failure-atomic `ident`, `identPath`, `identSegments`, and owned text/blob `bind`; `rawUnsafe` | exhaustive/per-boundary state and retry tests for both bind dialects, quoted-identifier growth, and invalid later path segments |
