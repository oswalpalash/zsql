...
| Transactions and savepoints | Complete | explicit `transactionOpen` boundaries; nested/idle/aborted states; typed PostgreSQL isolation/access/deferrable options; PostgreSQL `25P02` mapping; prepared-statement transition safety; failed-state savepoint recovery; failed SQLite best-effort rollback retains retryable transaction ownership; cross-driver bounded savepoint IDs; direct and pooled failure-scoped `withSavepoint`; `Tx(D)`, `Savepoint(D)`, `withTx` | SQLite state/rollback-retry/savepoint-exhaustion plus cross-driver savepoint-exhaustion/transaction-view tests and deterministic direct/pool scoped-savepoint tests and PostgreSQL direct/pool/live scoped-savepoint and typed-transaction-option tests |
