...
| Safe parameter binding | Complete | positional/named APIs; prevalidated single-resolution SQLite named indices with unbounded marker probing and exact OOM errors; single-allocation PostgreSQL Bind packets; full unsigned 16-bit parameter count; strict protocol C-strings | long-name/allocation/failure-atomicity, byte-parity, NUL, driver, and PostgreSQL live tests |
