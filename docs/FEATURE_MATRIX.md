...
| Query builder | Complete | `QueryBuilder`; reusable failure-free reset; failure-atomic `ident`, `identPath`, `identSegments`, owned text/blob `bind`, all-at-once `bindAll`, and delimiter-aware atomic `bindJoined`; `rawUnsafe` | exhaustive/per-boundary state and retry tests for both bind dialects, quoted-identifier growth, and invalid later path segments |
