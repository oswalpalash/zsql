...
| Query builder | Complete | `QueryBuilder`; reusable failure-free reset; failure-atomic `ident`, `identPath`, `identSegments`, owned text/blob `bind`, and all-at-once `bindAll`; `rawUnsafe` | exhaustive/per-boundary state and retry tests for both bind dialects, quoted-identifier growth, and invalid later path segments |
