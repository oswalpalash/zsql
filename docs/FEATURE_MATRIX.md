...
| Native PostgreSQL driver | Complete | full-open `connect_timeout`; endpoint-owning `CancelHandle`; lifecycle-aware command/COPY/notification recovery | cancellation endpoint/OOM tests; live post-connection handle teardown and cancellation/COPY/notification recovery in CI |
