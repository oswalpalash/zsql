...
| Native artifact reproducibility | CLI complete; static archive under investigation | public `-Dstrip=true` install mode removes build-cache debug paths; Zig 0.16 Linux archive bytes still vary across isolated cache roots | `reproducibility-smoke` compares CLI bytes and requires both static archives across separate local caches and prefixes |
