...
| Native PostgreSQL driver | Complete | full-open `connect_timeout`; endpoint-owning `CancelHandle`; owned notifications; pool listeners clear subscriptions before lease release; lifecycle-aware command/COPY recovery | cancellation/notification ownership and OOM tests; live post-connection teardown, clean listener reuse, cancellation/COPY/notification recovery in CI |
