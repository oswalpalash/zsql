...
| Prepared statements | Complete | schema-aware SQLite cache; PostgreSQL Parse/plan-error cache recovery; explicit SQLite `Conn.prepare` | SQLite schema-change tests and PostgreSQL live invalidation tests |
...
| Migrations | Complete | transactional apply, durable dirty failures, checksum validation; `Migrator(D).up`, `.status`; CLI commands | SQLite failure injection, PostgreSQL live tests, migration example |
...
| Rich database errors | Complete | fine-grained constraints; connection-owned SQLite/PostgreSQL `lastError`; `DbError`, `OwnedDbError` | SQLite redaction/extended-code tests; SQLSTATE and live constraint tests |
