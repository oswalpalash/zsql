...
| Prepared statements | Complete | driver-selected `Statement(D)`; reusable ownership-safe SQLite query borrows with busy/deferred-close state; explicit PostgreSQL direct/pool owners; owned prepared-query rows survive statement/pool teardown; reusable named-bind scratch; session-monotonic PostgreSQL names; allocation-free cache teardown; unsigned OID metadata; narrow invalidation recovery | external façade contract; SQLite reuse/OOM/teardown/schema-change tests and PostgreSQL protocol/direct/pool/cache/OOM/lifetime tests |
| Safe parameter binding | Complete | positional/named APIs; unbounded one-buffer SQLite bare-name marker probing with exact OOM errors; single-allocation PostgreSQL Bind packets; full unsigned 16-bit parameter count; strict protocol C-strings | long-name/allocation, byte-parity, NUL, driver, and PostgreSQL live tests |
