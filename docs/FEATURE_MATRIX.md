...
| Safe parameter binding | Complete | positional/named APIs; two-allocation SQLite named-index resolution with one unbounded shared marker probe and exact OOM errors; single-allocation PostgreSQL Bind packets; typed canonical UUID and explicit ISO date/time/UTC-timestamp binding with optional-null support; full unsigned 16-bit parameter count; strict protocol C-strings | long-name/allocation-boundary/failure-atomicity, typed-UUID/OOM, temporal-format/state/OOM, byte-parity, NUL, driver, and PostgreSQL live tests |
| Typed row decoding | Complete | `Row.as`, `Row.asName`, `Row.to`, `zsql.decode`; strict allocation-free opt-in ISO temporal parsers/formatters with signed/expanded years; allocation-free struct mapping without arbitrary result-width caps; owned PostgreSQL bytea hex/escape decode; single-schema query contract; failure-atomic PostgreSQL result finalization | temporal valid/invalid/precision/expanded-year/round-trip tests; QueryBuilder joined-binding exhaustive-OOM tests; malformed/OOM unit tests; forced column-name append/final-transfer failures; wide-result mapping, multi-result recovery, and core/live driver tests |
...
| Query builder | Complete | `QueryBuilder`; reusable failure-free reset; failure-atomic `ident`, `identPath`, `identSegments`, joined identifiers, owned text/blob `bind`, all-at-once `bindAll`, delimiter-aware atomic `bindJoined`, typed canonical `bindUuid`, and explicit temporal `bindDate`/`bindTime`/`bindTimestampUtc`; `rawUnsafe` | exhaustive/per-boundary state and retry tests for both bind dialects, quoted-identifier growth, invalid later path segments, UUID formatting/ownership, and temporal format/mutation/OOM coverage |
