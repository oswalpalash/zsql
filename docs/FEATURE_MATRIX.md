...
| Optional offline query checks | Complete within documented bounded scope | `zsql.check`, `zsql.checkedQuery`; exact bind contracts; projection/alias-bound row shapes; sound portable COUNT/MIN/MAX inference; explicit scope-capacity failure; typed domain wrappers; authoritative numeric width checks | parser/projection/aggregate/scope/shape/type/nullability/narrowing tests; `zig build check-sql` |
