...
| Optional offline query checks | Complete within documented bounded scope | `zsql.check`, `zsql.checkedQuery`; exact bind contracts; projection/alias-bound row shapes; typed domain wrappers; authoritative numeric width checks | parser/projection/shape/type/narrowing tests; `zig build check-sql` |
