...
| Connection pooling | Complete | `Pool(D)`, `Lease(D)`, health-aware consuming release, OOM-safe idle return and owned-result unwind, failed-open capacity wakeup, shutdown draining/wakeup, stats and timeouts | Cross-driver deterministic failed-open infinite-waiter tests; SQLite release-OOM/result-unwind/recovery tests; PostgreSQL live release-OOM/result-unwind/shutdown tests |
