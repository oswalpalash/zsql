...
| Migrations | Complete | transactional apply with under-lock history revalidation, durable dirty failures with explicit persistence-error precedence, checksum-guarded API/CLI repair; allocator-owned post-connection `MigrationStatus`; exclusive atomic and collision-retrying `migrate new`; checked `u64` version discovery; `Migrator(D).up`, `.status` | SQLite begin-immediate history-read contract; cross-driver status OOM/teardown tests; forced SQLite and live PostgreSQL dirty-write failures; SQLite repair workflow; PostgreSQL live repair tests; CLI atomic-create/concurrent-collision/version-boundary/parser tests; migration example |
