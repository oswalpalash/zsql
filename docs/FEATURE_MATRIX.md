...
| Migrations | Complete | strict ordered/complete plan validation; transactional apply with under-lock history revalidation; verified failure-safe PostgreSQL advisory-lock release; durable dirty failures with explicit persistence-error precedence; checksum-guarded API/CLI repair; allocator-owned post-connection `MigrationStatus`; exclusive atomic and collision-retrying `migrate new`; checked `u64` version discovery; `Migrator(D).up`, `.status` | Shared plan-order/history tests; SQLite pre-SQL out-of-order rejection; live PostgreSQL incomplete/out-of-order and forced unlock-OOM/session-release contracts; begin-immediate history-read contract; cross-driver status OOM/teardown tests; forced dirty-write failures and repair workflows; CLI atomic-create/concurrent-collision/version-boundary/parser tests; migration example |
