...
| Transactions and savepoints | Complete | explicit nested/idle/aborted states; PostgreSQL `25P02` mapping; prepared-statement transition safety; failed-state savepoint recovery; failed SQLite best-effort rollback retains retryable transaction ownership; bounded savepoint IDs; failure-scoped `withSavepoint`; `Tx(D)`, `Savepoint(D)`, `withTx` | SQLite state/rollback-retry/savepoint-exhaustion and scoped-savepoint tests plus PostgreSQL direct/prepared/pool/live scoped-savepoint tests |
