...
| Safe parameter binding | Complete | positional/named APIs; two-allocation SQLite named-index resolution with one unbounded shared marker probe and exact OOM errors; single-allocation PostgreSQL Bind packets; typed canonical UUID binding with optional-null support; full unsigned 16-bit parameter count; strict protocol C-strings | long-name/allocation-boundary/failure-atomicity, typed-UUID/OOM, byte-parity, NUL, driver, and PostgreSQL live tests |
...
| Query builder | Complete | `QueryBuilder`; reusable failure-free reset; failure-atomic `ident`, `identPath`, `identSegments`, joined identifiers, owned text/blob `bind`, all-at-once `bindAll`, delimiter-aware atomic `bindJoined`, and typed canonical `bindUuid`; `rawUnsafe` | exhaustive/per-boundary state and retry tests for both bind dialects, quoted-identifier growth, invalid later path segments, and UUID formatting/ownership |
