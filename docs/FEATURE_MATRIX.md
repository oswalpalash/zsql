...
| Migrations | Complete | transactional apply, durable dirty failures, checksum-guarded API/CLI repair; allocator-owned post-connection `MigrationStatus`; exclusive atomic `migrate new`; `Migrator(D).up`, `.status` | cross-driver status OOM/teardown tests, SQLite repair workflow, PostgreSQL live repair tests, CLI atomic-create/parser tests, migration example |
