...
pub const Pool = struct {
    ...
    /// Acquire a lease and run `body` in a transaction started with typed
    /// PostgreSQL characteristics. Lease health accounting matches `withTx`.
    pub fn withTxWithOptions(
        self: *Pool,
        ctx: anytype,
        options: conn_mod.TransactionOptions,
        ...
        (try lease.conn()).withTxWithOptions(runner, options, Runner.run) catch |err| {
            lease.finishAfterError(err);
            return err;
        };
        try lease.release();
