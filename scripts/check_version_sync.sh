...
build_version=$(sed -n 's/^[[:space:]]*const package_version[[:space:]]*=[[:space:]]*"\([^"]*\)";[[:space:]]*$/\1/p' "$build_file")
...
if [ -z "$build_version" ] || [ "$(printf '%s\n' "$build_version" | wc -l | tr -d ' ')" -ne 1 ]; then
    echo "could not extract exactly one package_version constant from $build_file" >&2
    exit 1
fi
